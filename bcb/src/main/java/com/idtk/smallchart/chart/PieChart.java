...
public class PieChart extends PieRadarChart<IPieData> implements IPieChart {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		float minLength = Math.min(mWidth, mHeight);//宽高中最小值
		float userSetLength = mPieAxisData.getAxisLength();//用户设置的值
		radius = minLength * 0.4f;//直径默认设为为宽高中最小值的0.8
		if (userSetLength > 0) {
			radius = userSetLength;
			if (radius > minLength * 0.5f) {//直径不能大于宽高中的最小值
	...
	protected void animated() {
		if (!isAnimated) {
			...
			mChartAnimator.animatedY(mPieAxisData.getAnimatTime(), 360f);
		}
	}
	...
	public void setAnimatTime(long animatTime) {
		mPieAxisData.setAnimatTime(animatTime);
	}

	@Override
	public void setAxisLength(float axisLength) {
		mPieAxisData.setAxisLength(axisLength);
	}

	@Override
	public void setOffsetAngle(float offsetAngle) {
		mPieAxisData.setOffsetAngle(offsetAngle);
	}
}