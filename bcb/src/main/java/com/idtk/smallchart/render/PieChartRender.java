...
public class PieChartRender extends ChartRender implements TouchListener {
	...
	public void drawGraph(Canvas canvas, float animatedValue) {
		if (Math.min(pieData.getAngle() - 1, animatedValue - pieData.getCurrentAngle()) >= 0) {
			drawAngle = Math.min(pieData.getAngle() - pieAxisData.getOffsetAngle(), animatedValue - pieData.getCurrentAngle());
		} else {
			drawAngle = 0;
		}
		if (touchFlag) {
			drawArc(canvas, pieData.getCurrentAngle(), drawAngle, pieData, pieAxisData.getOffsetRectFs()[0], pieAxisData.getOffsetRectFs()[1],
					pieAxisData.getOffsetRectFs()[2], mPaint);
		} else {
			drawArc(canvas, pieData.getCurrentAngle(), drawAngle, pieData, pieAxisData.getRectFs()[0], pieAxisData.getRectFs()[1], pieAxisData.getRectFs()[2],
					mPaint);
		}
	}
	...
	private void drawText(Canvas canvas, IPieData pie, float currentStartAngle, NumberFormat numberFormat, boolean flag) {
		int textPathX = (int) (Math.cos(Math.toRadians(currentStartAngle + (pie.getAngle() / 2))) * pieAxisData.getAxisLength()
				* (1 + pieAxisData.getOutsideRadiusScale()) / 2);
		int textPathY = (int) (Math.sin(Math.toRadians(currentStartAngle + (pie.getAngle() / 2))) * pieAxisData.getAxisLength()
				* (1 + pieAxisData.getOutsideRadiusScale()) / 2);
		mPointF.x = textPathX;
		mPointF.y = textPathY;
		String[] strings;//这里控制扇形图显示的文本信息
		if (flag) {//绘制名称和百分比
