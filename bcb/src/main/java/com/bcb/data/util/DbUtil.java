...
public class DbUtil {
    ...
    public static WelfareBean getWelfare() {
        ...
        try {
            beanList = DataSupport.where("username like ? and openDate like ?", App.saveUserInfo.getLocalPhone(),
    ...
    public static UserExtraInfo getUserExtra() {
        ...
        try {
            beanList = DataSupport.where("username like ?", App.saveUserInfo.getLocalPhone()).find(UserExtraInfo.class);
        }catch (Exception e){
            e.printStackTrace();
        }
        if (null != beanList && beanList.size() > 0){
