...
public class Activity_LoanRequest_Job extends Activity_Base implements TextWatcher {
    ...
    @BindView(R.id.loan_office)
    TextView loan_office;//工作单位全称
    @BindView(R.id.loan_jobs)
    EditText loan_jobs;//工作职位
    @BindView(R.id.loan_department)
    EditText loan_department;    //所在部门
    @BindView(R.id.loan_work_experience)
    EditTextWithDate loan_work_experience;//入职时间
    @BindView(R.id.loan_office_address)
    EditText loan_office_address;    //办公地点
    @BindView(R.id.loan_earn)
    EditText loan_earn;    //月均收入
    @BindView(R.id.loan_earn_total)
    EditText loan_earn_total;    //年税后总收入
