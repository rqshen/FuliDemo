...
public class _ProductFragment extends BaseFragment implements OnClickListener {
	
	private static final String TAG = "_ProductFragment";
	...
	public _ProductFragment(Context ctx, String CompanyId) {
		super();
		this.ctx = ctx;
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		requestQueue = MyApplication.getInstance()
				.getRequestQueue();
		//注册广播
			...
			public void onResponse(JSONObject response) {
				...
				try {
					...
					if (PackageUtil.getRequestStatus(response, ctx)) {
						...
						//判断JSON对象是否为空
						if (obj != null) {
							productListBean = MyApplication.mGson.fromJson(obj.toString(), ProductListBean.class);
						}
						//存在产品列表
	...
	class Receiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			if (intent.getAction()
				...
				MyApplication.saveUserInfo.setCurrentCompanyId(CompanyId);
				MyApplication.saveUserInfo.setCurrentCompanyName(CompanyName);
				refreshLayout.autoRefresh();
			}
		}
	}
