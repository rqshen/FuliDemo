...
//			}
			...
			public void onResponse(JSONObject response) {
				try {
					...
					//去登陆
						...
						startActivity(new Intent(Activity_Project_Buy.this, LoginActivity.class));
					}
				} catch (Exception e) {
					e.printStackTrace();
				}
	...
	private void clickButton() {
		...
		if (mSimpleProjectDetail == null) {
			ToastUtil.alert(Activity_Project_Buy.this, "无法获取买标数据，请稍后重试");
			return;
		}
		//项目可投金额是否大于0元
		...
		if (MyApplication.mUserDetailInfo == null || !MyApplication.mUserDetailInfo.HasOpenCustody) {
			startActivity(new Intent(Activity_Project_Buy.this, FundCustodianAboutActivity.class));
			return;
		}
		//未输入投资金额
		...
		if (invest_money.getText().toString().startsWith(".")) {
			Toast.makeText(Activity_Project_Buy.this, "您的输入有误", Toast.LENGTH_SHORT).show();
			return;
		}
		//输入金额
				...
				if (PackageUtil.getRequestStatus(response, Activity_Project_Buy.this)) {
					try {
						...
						if (result != null) {
							...
							//跳转到webview
							FundCustodianWebActivity.launche(Activity_Project_Buy.this, "投资确认", postUrl, postData);
						}
					} catch (Exception e) {
	...
	private void rechargeMoney() {
		...
		//已开通托管
		if (MyApplication.mUserDetailInfo != null && MyApplication.mUserDetailInfo.HasOpenCustody)
			startActivity(new Intent(Activity_Project_Buy.this, RechargeActivity.class));
		else startActivity(new Intent(Activity_Project_Buy.this, FundCustodianAboutActivity.class));
	}
