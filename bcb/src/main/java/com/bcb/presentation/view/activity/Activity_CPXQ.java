 ...
 */
public class Activity_CPXQ extends Activity_Base implements View.OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		UmengUtil.eventById(Activity_CPXQ.this, R.string.buy_home);
		//初始化界面
		setupView();
		//加载普通标的数据
		loadProjectData();
	...
	private void loadProjectData() {
		...
		BcbJsonRequest jsonRequest = new BcbJsonRequest(url, jObject, TokenUtil.getEncodeToken(this), new BcbRequest
			...
			public void onResponse(JSONObject response) {
				...
				if (type == 1 || type == 2 || PackageUtil.getRequestStatus(response, Activity_CPXQ.this)) {
			...
			public void onErrorResponse(Exception error) {
				ToastUtil.alert(Activity_CPXQ.this, "网络异常，请稍后重试");
				hideProgressBar();
			}
		});
	...
	private void autoOpen() {
		...
		String encodeToken = TokenUtil.getEncodeToken(Activity_CPXQ.this);
		...
		BcbJsonRequest jsonRequest = new BcbJsonRequest(requestUrl, obj, encodeToken, true, new BcbRequest.BcbCallBack<JSONObject>() {
			...
			public void onResponse(JSONObject response) {
				...
				if (PackageUtil.getRequestStatus(response, Activity_CPXQ.this)) {
					try {
						...
						if (result != null) {
							...
							Activity_WebView.launche(Activity_CPXQ.this, "开启份额锁", postUrl, postData);
						}
					} catch (Exception e) {
						LogUtil.d("bqt", "开通自动投标2" + e.getMessage());
					}
				} else if (response != null) {
					Toast.makeText(Activity_CPXQ.this, response.optString("message"), Toast.LENGTH_SHORT)
							.show();
				}
			}
	...
	private void loadUserDetailInfoData() {
		...
		if (App.saveUserInfo.getAccess_Token() == null || App.mUserDetailInfo != null && App.mUserDetailInfo.BankCard != null) {
			...
			BcbJsonRequest jsonRequest = new BcbJsonRequest(UrlsTwo.UserMessage, null, TokenUtil.getEncodeToken(this), new BcbRequest
				...
				@Override
				public void onErrorResponse(Exception error) {
					ToastUtil.alert(Activity_CPXQ.this, "网络异常，请稍后再试");
				}
			});
	...
	private void showAdviserTips(String tips) {
		dialogWidget = new DialogWidget(Activity_CPXQ.this, getAdviserView(tips), true);
		dialogWidget.show();
	}

	protected View getAdviserView(String tips) {
		return MyMaskFullScreenView.getInstance(Activity_CPXQ.this, tips, new MyMaskFullScreenView
				.OnClikListener() {
			@Override
	...
	public void onBackPressed() {
		UmengUtil.eventById(Activity_CPXQ.this, R.string.bid_buy_back);
		finish();
	}
		...
		switch (v.getId()) {
			...
			case R.id.layout_investment:
				if (mSimpleProjectDetail != null && mSimpleProjectDetail.Status != 20) {
					UmengUtil.eventById(Activity_CPXQ.this, R.string.bid_buy_info2);
				}
			...
			case R.id.layout_customer_service:
				UmengUtil.eventById(Activity_CPXQ.this, R.string.bid_buy_kefu);
				//如果ID存在
				String userId = null;
			...
			case R.id.button_buy:
				if (mSimpleProjectDetail != null && mSimpleProjectDetail.Status != 20) {
					UmengUtil.eventById(Activity_CPXQ.this, R.string.bid_buy_act);
				}
				clickButton();
				break;
			...
			case R.id.layout_company_hr:
				if (mSimpleProjectDetail != null && mSimpleProjectDetail.Status != 20) {
					UmengUtil.eventById(Activity_CPXQ.this, R.string.bid_buy_info);
				}
			...
			case R.id.layout_project_detail:
				if (mSimpleProjectDetail != null && mSimpleProjectDetail.Status != 20) {
					UmengUtil.eventById(Activity_CPXQ.this, R.string.bid_buy_detail1);
				}
				if (null != mSimpleProjectDetail && !TextUtils.isEmpty(mSimpleProjectDetail.PageUrl)) {
					Activity_Browser.launche2(Activity_CPXQ.this, mSimpleProjectDetail.Name,
							mSimpleProjectDetail.PageUrl + "&tab=0", 20094);
					//							"http://192.168.1.200:7073/", 20094);
				}
				break;
			//保障信息
			case R.id.layout_security:
				if (mSimpleProjectDetail != null && mSimpleProjectDetail.Status != 20) {
					UmengUtil.eventById(Activity_CPXQ.this, R.string.bid_buy_detail2);
				}
				if (null != mSimpleProjectDetail && !TextUtils.isEmpty(mSimpleProjectDetail.PageUrl)) {
					Activity_Browser.launche2(Activity_CPXQ.this, mSimpleProjectDetail.Name,
							mSimpleProjectDetail.PageUrl + "&tab=1", 20094);
					//							"http://192.168.1.200:7073/", 20094);
				}
				break;
			//证明文件
			case R.id.layout_identify:
				if (mSimpleProjectDetail != null && mSimpleProjectDetail.Status != 20) {
					UmengUtil.eventById(Activity_CPXQ.this, R.string.bid_buy_detail3);
				}
				if (null != mSimpleProjectDetail && !TextUtils.isEmpty(mSimpleProjectDetail.PageUrl)) {
					Activity_Browser.launche2(Activity_CPXQ.this, mSimpleProjectDetail.Name,
							mSimpleProjectDetail.PageUrl + "&tab=3", 20094);
					//							"http://192.168.1.200:7073/", 20094);
				}
				break;
			//投资列表
			case R.id.layout_invest_list:
				if (mSimpleProjectDetail != null && mSimpleProjectDetail.Status != 20) {
					UmengUtil.eventById(Activity_CPXQ.this, R.string.bid_buy_detail4);
				}
				if (null != mSimpleProjectDetail && !TextUtils.isEmpty(mSimpleProjectDetail.PageUrl)) {
					Activity_Browser.launche2(Activity_CPXQ.this, mSimpleProjectDetail.Name,
							mSimpleProjectDetail.PageUrl + "&tab=2", 20094);
					//							"http://192.168.1.200:7073/", 20094);
				}
				break;
		}
	...
	private void clickButton() {
		...
		if (mSimpleProjectDetail == null) {
			ToastUtil.alert(Activity_CPXQ.this, "无法获取买标数据");
			return;
		}

		//判断可投金额是否大于0元
		if (mSimpleProjectDetail.Balance <= 0) {
			ToastUtil.alert(Activity_CPXQ.this, "可投金额为0元，该标不能投");
			return;
		}

		//还没有登陆时，跳转至登陆页面
		if (App.saveUserInfo.getAccess_Token() == null) {
			startActivity(new Intent(Activity_CPXQ.this, Activity_Login.class));
			finish();
			return;
		}
