...
import com.bcb.module.myinfo.financial.FinancialActivity;
...
//			layout_update_line.setVisibility(View.GONE);
	...
	private void showData() {
		...
		if (MyApplication.saveUserInfo.getAccess_Token() == null) {
			...
			if(MyApplication.mUserDetailInfo!=null)value_yhq.setText(MyApplication.mUserDetailInfo.CouponCount + "张");
	...
	public void onClick(View v) {
		if (v.getId() == R.id.layout_update) {
			if (MyApplication.isNeedUpdate && MyApplication.versionBean != null) showVersionDialog2();
		...
		switch (v.getId()) {
			...
			//理财
			case R.id.rl_lc:
				startActivity(new Intent(ctx, FinancialActivity.class));
				break;
			//优惠券
			case R.id.rl_yhq:
				if (isLoading()) return;
	...
	private void toJoinCompany() {
		...
		//否则需要判断MyCompany字段
			...
			//审核中
			else if (MyApplication.mUserDetailInfo.MyCompany.Status == 5) {
				companyAlertView("您的认证申请正在审核", "预计2个工作日内完成，请耐心等候");
			}
			//拉黑
			else if (MyApplication.mUserDetailInfo.MyCompany.Status == 15) {
